...
export const useUsuarioStore = defineStore("usuario", {
    ...
    actions: {
        ...
        async cambiarRole(id, establecimientoid) {
            const respuesta = await axios.post('cambiar-role', { id: id, establecimientoid: establecimientoid })
            if (respuesta.data) {
                localStorage.setItem('userSession', respuesta.data);
                window.location.href = '/';
            }
        },
        async cambiarEstablecimiento(id, roleid) {
            const respuesta = await axios.post('cambiar-establecimiento', { id: id, roleid: roleid })
            if (respuesta.data) {
                localStorage.setItem('userSession', respuesta.data);
                window.location.href = '/';
            }
